...
foreach ($researches as $research) {
    if (!$research->latitude || !$research->longitude) {
        $missing++;
        $matched = false;

        foreach ($provinces as $name => $coords) {
            if (mb_strpos($research->title, $name) !== false || mb_strpos($research->result_publication, $name) !== false) {
                ...
                $research->latitude = $lat;
                $research->longitude = $lng;
                $matched = true;
                echo "Matched: {$name} -> {$research->title}\n";
                break;
            }
        }
        ...
        if (!$matched) {
            ...
            $research->latitude = $lat;
            $research->longitude = $lng;
            echo "Defaulted: {$research->title}\n";
        }

        if ($research->save(false)) {
            $updated++;
        }
    }
}
