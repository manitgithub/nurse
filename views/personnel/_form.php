...
<div class="max-w-3xl mx-auto bg-white shadow-sm rounded-xl p-6 border border-gray-200">
    ...
    <div class="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block text-xs font-bold text-gray-500 mb-1 uppercase">ใบอนุญาตประกอบวิชาชีพ (ไฟล์แนบ)</label>
            ...
            <div>
                ...
                <?php if ($model->member_card_file): ?>
                    <div class="text-[10px] text-gray-500 mt-1">ไฟล์ปัจจุบัน: <?= basename($model->member_card_file) ?></div>
                <?php endif; ?>
            </div>
        </div>
    </div>
