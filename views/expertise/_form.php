...
<div class="max-w-lg mx-auto bg-white shadow-sm rounded-xl p-6 border border-gray-200">
    ...
    <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">ชื่อความเชี่ยวชาญ <span class="text-red-500">*</span></label>
        <?= $form->field($model, 'name')->textInput(['class' => $inputClass, 'maxlength' => true])->label(false) ?>
    </div>
    <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">สถานะ</label>
        <?= $form->field($model, 'status')->dropDownList([1 => 'เปิดใช้งาน', 0 => 'ปิดใช้งาน'], ['class' => $inputClass])->label(false) ?>
    </div>
