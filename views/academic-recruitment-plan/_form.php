...
<div class="max-w-lg mx-auto bg-white shadow-sm rounded-xl p-6 border border-gray-200">
    ...
    <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">ปีงบประมาณ <span
                class="text-red-500">*</span></label>
        <?= $form->field($model, 'fiscal_year')->textInput(['class' => $inputClass])->label(false) ?>
    </div>
    <div class="grid grid-cols-2 gap-4">
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">อัตราที่ได้รับ</label>
            <?= $form->field($model, 'quota_amount')->textInput(['class' => $inputClass, 'type' => 'number', 'min' => 0])->label(false) ?>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">อัตราที่บรรจุแล้ว</label>
            <?= $form->field($model, 'recruited_amount')->textInput(['class' => $inputClass, 'type' => 'number', 'min' => 0])->label(false) ?>
        </div>
    </div>
